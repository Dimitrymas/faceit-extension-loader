...
// Generated from electron-chrome-extensions/browser-action so FACEIT's sandboxed
// preload does not need to resolve npm modules at runtime.
(() => {
  ...
  function logRenderer(message, details) {
    try {
      ...
      if (ipcRenderer && typeof ipcRenderer.send === 'function') {
        ipcRenderer.send(IPC_RENDERER_LOG, {
          message,
          details,
          href: location.href,
  ...
  try {
    ...
    if (!module.exports || typeof module.exports.injectBrowserAction !== 'function') {
      throw new Error('electron-chrome-extensions/browser-action export is unavailable');
    }

    module.exports.injectBrowserAction();
    logRenderer('browserAction preload injected');
  } catch (error) {
    logRenderer('browserAction preload injection failed', serializeError(error));
    console.warn('[faceit-extension-loader] failed to inject browserAction preload', error);
  }
})();
