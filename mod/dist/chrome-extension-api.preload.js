...
(() => {
  ...
  var injectExtensionAPIs = () => {
    ...
    function mainWorldScript() {
      ...
      const chrome = cloneChromeObject(nativeChrome);
      ...
      function cloneChromeObject(source) {
        const clone = {};
        if (!source || typeof source !== "object") {
          return clone;
        }
        for (const key of Object.getOwnPropertyNames(source)) {
          if (key === "management") continue;
          try {
          ...
          factory: (base) => {
            ...
            if (base && typeof base === "object") {
              for (const key of Object.keys(base)) {
                if (key === "sync" || key === "managed") continue;
                try {
                  safeBase[key] = base[key];
                } catch {
                }
              }
            }
            const local = safeBase.local;
            return {
              ...safeBase,
              // Electron can expose chrome.storage.sync as a throwing getter.
              // Keep sync/managed as local-backed fallbacks without reading them.
              managed: local,
              sync: local
            };
          }
