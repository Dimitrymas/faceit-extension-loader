...
test('native Windows setup uses the FACEIT Electron runtime and stays current-user scoped', () => {
  ...
  assert.match(installer, /FACEIT\.exe closes automatically when setup starts/);
  ...
  assert.match(installer, /SetWindowTextW\(g_install, g_install_complete \? L"Launch FACEIT"/);
  assert.equal(installer.includes('MessageBoxW(window, message_text, APP_TITLE, MB_OK | MB_ICONINFORMATION)'), false);
